...
MARK, 
COLOR, 
COUNT(COLOR) AS 'qts by mark' 
FROM AUTO
WHERE MARK = 'LADA'
GROUP BY  MARK, COLOR 
UNION SELECT 
MARK, 
COLOR, COUNT(COLOR) AS 'qts by mark' 
FROM AUTO
WHERE MARK = 'BMW'
GROUP BY  MARK, COLOR;
...
SELECT 
  MARK,
  (SELECT COUNT(*)  FROM auto) AS namecounter
 FROM auto;

SELECT DISTINCT a.MARK , 
       b.qts_by_mark,
       c.total -  b.qts_by_mark AS total
FROM auto a LEFT JOIN 
(
  SELECT MARK, 
	COUNT(color) AS qts_by_mark
	FROM auto
	GROUP BY MARK
) b 
  ON a.MARK = b.MARK 
...
WHERE EXISTS (SELECT * FROM test_b b WHERE a.id = b.id);
